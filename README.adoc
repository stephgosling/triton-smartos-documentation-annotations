...
Edgecast customer docs)] but somewhat applicable to those who self-host.
...
TIP: users (including operators it seems) can only see their own virtual
machines so, for example, the admin user can only see Triton components.
...
A package is a collection of properties, limits and boundaries which, when
coupled with a virtual machine image can constitute a VM. These properties are
things like disk quotas or vCPU allocations, memory limits etc. The https://github.com/TritonDataCenter/sdc-papi/blob/master/docs/index.md[Package API]
...
== Guest workloads, virtualisation and terminology

Naming here can be ambiguous, especially when contrasting with other hypervisors
or cloud solutions, so it's important to be clear and consistent with the
terminology.

* In illumos the unit of isolation is always the zone. `zones(7)` are more
featureful than both BSD Jails and Linux 'containers'. They can be thought of as
containers but that is a particularly confusing label, especially when
considering docker workloads.
* SmartOS builds an abstraction ontop of `zones(7)` that it calls Virtual
Machines (hence `vmadm`, VMAPI etc.) so whilst working with any guest workload
the tooling one interacts with is always labelled 'VM' in some way:
** OS Virtual Machines tend to be referred to as 'zones' still, sometimes with a
prefix: for example SmartOS zones are often also called 'native zones'. illumos
the concept of `brands(7)` (c.f. Personalities in Windows NT) where the kernel
provides syscall emulation of another operating system, such as `lx(7)`. Such
zones are refered to as 'branded zones' e.g. specifically in that case,
'lx zones'
** Workloads that leverage a Virtual Machine Monitor (VMM) are called Hardware
Virtual Machines (HVMs) or Guests. SmartOS supports both KVM and bhyve as VMMs.
Bhyve is considerably more performant, supports flexible disk storage and VM
resizing which makes it now the defacto choice; KVM is considered deprecated.
** Instances (in Triton specifically) refer to the native zones that make up the
control-plane, particularly pertinent in an HA-headnode environment

=== Triton Cloud Images

Edgecast maintain a repository of images in the public cloud
(images.smartos.org) containing:
...
The SmartOS/Triton documentation is still centred around KVM rather than Bhyve.
The process is similar however the `vmadm` documentation is out of date: it
...
+
----
[root@headnode (cabin) /var/tmp]# mkdir talos-1.14.0
[root@headnode (cabin) /var/tmp]# cd talos-1.14.0/

----
. Fetch and decompress the source image:
+
----
[root@headnode (cabin) /var/tmp/talos-1.14.0]# wget https://factory.talos.dev/image/376567988ad370138ad8b2698212367b8edcb69b5fd68c80be1f2ec7d603b4ba/v1.14.0/metal-amd64.raw.zst --no-check-certificate
[root@headnode (cabin) /var/tmp/talos-1.14.0]# zstd -d metal-amd64.raw.zst
metal-amd64.raw.zst : 4453302272 bytes
----

. Create a VM definition:
+
----
[root@headnode (cabin) /var/tmp/talos-1.14.0]# cat empty.vmdef.json
{
  "name": "empty",
  "brand": "bhyve",
  "vcpus": 1,
...
. Create an empty VM from the definition:
+
----
[root@headnode (cabin) /var/tmp/talos-1.14.0]# vmadm create -f empty.vmdef.json
Successfully created VM f34b4e1a-3ed5-440e-a0bd-c9e7c28606de
----

. Noting the zone ID, copy the disk image to the raw device of the empty VM:

+
----
[root@headnode (cabin) /var/tmp/talos-1.14.0]# dd if=metal-amd64.raw of=/dev/zvol/rdsk/zones/f34b4e1a-3ed5-440e-a0bd-c9e7c28606de/disk0 bs=16M
265+1 records in
265+1 records out
4453302272 bytes (4.1 GiB) transferred in 63.062095 secs (67 MiB/sec)
----
+
TIP: at this point you might boot the VM to make additional changes e.g.
configuration management or installing guest tools etc. (see also cdrom
attachment).

. Retrieve the zvol uuid for the for the VM storage:
+
----
[root@headnode (cabin) /var/tmp/talos-1.14.0]# vmadm get f34b4e1a-3ed5-440e-a0bd-c9e7c28606de |  json -a disks | json -a zfs_filesystem
zones/f34b4e1a-3ed5-440e-a0bd-c9e7c28606de/disk0
----
...
+
----
[root@headnode (cabin) /var/tmp/talos-1.14.0]# zfs get quota zones/f34b4e1a-3ed5-440e-a0bd-c9e7c28606de
NAME                                        PROPERTY  VALUE  SOURCE
zones/f34b4e1a-3ed5-440e-a0bd-c9e7c28606de  quota     30.3G  local
[root@headnode (cabin) /var/tmp/talos-1.14.0]# zfs set quota=40G zones/f34b4e1a-3ed5-440e-a0bd-c9e7c28606de
[root@headnode (cabin) /var/tmp/talos-1.14.0]# zfs snapshot zones/f34b4e1a-3ed5-440e-a0bd-c9e7c28606de/disk0@dataset
[root@headnode (cabin) /var/tmp/talos-1.14.0]# zfs send zones/f34b4e1a-3ed5-440e-a0bd-c9e7c28606de/disk0@dataset | gzip > talos-1.14.0.zvol.gz
[root@headnode (cabin) /var/tmp/talos-1.14.0]# ls -l talos-1.14.0.zvol.gz
-rw-r--r--   1 root     root     261869777 Sep  8 12:38 talos-1.14.0.zvol.gz
----

. At this point the empty VM is no longer needed so can be removed:
+
----
[root@headnode (cabin) /var/tmp/talos-1.14.0]# vmadm destroy f34b4e1a-3ed5-440e-a0bd-c9e7c28606de
Successfully deleted VM f34b4e1a-3ed5-440e-a0bd-c9e7c28606de
----

. Create a IMGAPI manifest (called `talos-1.14.0.manifest.json` that describes
the desired image:
+
----
{
  ...
  "name": "talos-1.14.0",
  "description": "Talos Linux 1.14.0 (SDC v0.0.1)",
  "version": "0.0.1",
  "state": "active",
  "disabled": false,
  "public": true,
  ...
  "files": [
    {
      "sha1": "<from the output of sum -x sha1 talos-1.14.0.zvol.gz | cut -d' ' -f1>"
      "size": <from the output of ls -l talos-1.14.0.zvol.gz | awk '{ print $5 }'>,
      "compression": "gzip"
    }
  ],
  "requirements": {
  ...
  "image_size": <as specified in disks.size in empty.vmdef.json>,
  "disk_driver": "virtio",
  "nic_driver": "virtio",
  "cpu_type": "host"
}
...
+
TIP: The above, wrapped into a shell script looks like this:
----
#!/usr/bin/env bash

VMNAME=$(basename $(pwd))
SDCVER='0.0.2'
VMDESC="Talos Linux 1.14.0 (SDC ${SDCVER})"
OWNER_UUID=$(sdc-ldap s 'login=admin' | grep ^uuid | cut -d' ' -f2)
IMAGE_SIZE=10240
ZVOL_BLOCKSIZE=$(ls -l ${VMNAME}.zvol.gz | awk '{ print $5 }')
ZVOL_SHA1=$(sum -x sha1 ${VMNAME}.zvol.gz | cut -d' ' -f1)
...
+
----
[root@headnode (cabin) /var/tmp/talos-1.14.0]# sdc-imgadm import -m talos-1.14.0.manifest.json -f talos-1.14.0.zvol.gz
Imported image ed6bd1b0-ab85-11f1-8fa4-07421ff627ca (talos-1.14.0, 0.0.2, state=unactivated)
...1f1-8fa4-07421ff627ca [======================================>] 100% 249.74MB
Added file "talos-1.14.0.zvol.gz" (compression "gzip") to image ed6bd1b0-ab85-11f1-8fa4-07421ff627ca
Activated image ed6bd1b0-ab85-11f1-8fa4-07421ff627ca
----

The image will now be available for use.

=== Running Bhyve Virtual Machines

==== Console output from bhyve VMs

In order to access to the console of a bhyve VM via VNC, the VM must
...
For VMs not started/managed by Triton, the UEFI bootrom can be enabled on
a stopped VM directly via `vmadm(8)` on the given compute node:
...
Resizing virtual machines cannot be done via the Admin UI. There are however three ways
to change the CPU and memory limits for a virtual machine. All require the
VM to be in the stopped state.

. via `vmadm(8)`
. via the Triton VMAPI: `sdc sdc-vmapi /vms/<uuid>?action=update -d '{"billing_id":"<new package uuid>"}'`
...
parameter. This means when specifying a single disk of a given size, the VM will
need to be stopped and the disk resized (see below). While this is slightly
inconvenient it as the advantage of leaving space available for snapshots.
...
Although the Operations Portal supports storage resizing, as with resizing
virtual machines, it lags behind the triton CLI. Resizing a boot or data disk
can be done on the given Compute Node. In the examples below `X` is the linux
device, `Y` is the corresponding ZFS zvol, so `/dev/vda` mapping to `disk0`,
`/dev/vdb` to `disk1` etc.
...
. boot the VM again and `resize2fs /dev/vdX` to grow the filesystem to the
end of the device
...
==== Guests stuck in shutdown

One can inspect the guest state from the global zone first with `pstack $PID`
which will only show output of the guest is stuck somehow in userland. If it's
stuck in the kernel then this output will be empty/error. To inspect from the
kernel side you will need to use the modular debugger:
...
unix users matched what existed before/on other hosts):
...
which will at least stop the zone (therefore, the data) being destroyed. Doing
will however break migrations.
...
more advanced services such as CNS and docker. Whilst advice for the headnode
hardware is that it be the same/similar as compute nodes, it appears it cannot
participate in the underlay network (attempts to bring up the underlay NIC will
forever be stuck in the _provisioning_ state).
...
done at CN setup time and my (current) understanding is that HVM guests would
...
The headnode and standalone SmartOS can be configured to boot from a ZFS pool
rather than USB. This need not be the `zones` pool but can be. Community members
report that a typical use case for booting from a separate pool is to side-step
buggy firmware where given drives on a controller can't be booted from.
To enable:
...
For SmartOS, new Platform Images are managed via `piadm` (`piadm install
20260122T000642Z`, `piadm activate 20260122T000642Z` etc.) but for the headnode
these functions are performed with `sdcadm`
...
corrected by mounting the USB stick (`sdc-usbkey mount`, and must be done even
when booting from a ZFS pool) and editing the NIC tags in `/mnt/usbkey/config`.
Following a reboot Triton should start but NICs in NAPI will still need to be
cleaned up, particularly if reusing headnode hardware.
...
still be interrogated/altered with `sdc-oneachnode`.


= Installing the headnode/installing standalone SmartOS

The https://docs.tritondatacenter.com/private-cloud/install/headnode-installation[headnode
documentation] and https://docs.smartos.org/download-smartos/[standalone SmartOS
documentation] are complete but some points and steps that are worth
re-iterating:

* make absolutely sure that there is no semblance of anything ZFS related on
disks, it _will_ trip either installer up. Even recreating the partition table
in Linux is not guaranteed to clear everything. What _does_ work is to boot the
...
For Triton, a USB 3.0 stick takes approximately 10 minutes to copy the files.
If not booting from a pool use the fastest USB disk you can find as this can
save several minutes for each reboot.
* Once installed I perform the following:
...
** `pkgin install mozilla-rootcerts-openssl` to get modern CA chains
** Persist `/root` (see link:README.adoc#persisting-root[below])

== Miscellanea

=== Persisting /root

By default /root is populated in the ramdisk at boot which means that any files
there (such as `.bash_history`, SSH known_hosts etc.) are lost on reboot.
https://docs.smartos.org/persistent-root-dotfiles/[there are examples for
persisting the root dotfiles] but I prefer to overlay a different directory
(`/var/root` in my case) over the whole of `/root` using a similar technique:

. Create the new root (`/var` is its own ZFS dataset): `mkdir -m 0700 /var/root`
. Copy over the existing files:
+
----
cp -p /root/.bashrc /var/root/
cp -p /root/.profile /var/root/
cp -Rp /root/.ssh /var/root/
----

. Create a new SMF manifest in `/opt/custom/smf` describing a service that will perform the mount:
+
----
# cat /opt/custom/smf/root_home.xml
<?xml version='1.0'?>
<!DOCTYPE service_bundle SYSTEM '/usr/share/lib/xml/dtd/service_bundle.dtd.1'>
<service_bundle type='manifest' name='export'>
  <service name='site/root_home' type='service' version='0'>
    ...
    <method_context/>
    <exec_method name='start' type='method' exec='mount -O -F lofs /var/root /root' timeout_seconds='60'/>
    ...
    <template>
      <common_name>
        <loctext xml:lang='C'>Overlay mount /var/root on /root</loctext>
      </common_name>
    </template>
  </service>
</service_bundle>
...
. Import the service: `svccfg import /opt/custom/smf/root_home.xml`
. Check the status:
+
----
# svcs svc:/site/root_home
STATE          STIME    FMRI
online         19:03:13 svc:/site/root_home:default
----
. Logout and back in all SSH sessions (or reboot) for this to take effect.

=== Fix terminal sequences

SmartOS ships without bindings for `readline` for common sequences meaning
things like the Ctrl+Left/Right arrow word-jump keys don't work as they do on
Linux distributions. For example, the terminal emulator sends the xterm escape
sequence `\e[1;5D/` for these keys, but without the bindings `readline` consumes
the leading `\e[1` and echoes the remainder unparsed (in this case `;5D` for
Ctrl-Left).  To resolve, create an `.initrc` dotfile for root containing the
following:
