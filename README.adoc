...
command line tools in SmartOS, or API equivalents thereof in Triton:
...
PAPI features aren't well exposed in the console nor is there a native SmartOS
...
provisioning.
* the console only exposes a subset of the PAPI methods, which are rather rich:
...
be created (see image creation below). IMGAPI describes images and their
requirements via image manifests.

IMGAPI can be used via the console for searching for images or importing from
...
exposed in the console as well as with `vmadm(8)` and the cluster-wide
`sdc-vmadm`.
...
or created via the console do not specify a disk layout. This means that any
...
When specifying a disk layout in the console it is not possible to use the
...
To get AWS-like behaviour from a package, one must interact with PAPI directly
for example:
+
----
...
* for a host with more than one solid-state drive it will use them as individual
log devices rather than a mirror.
...
CAUTION: Note however once a log device has been added to a pool it cannot be
removed.
...
SCSI    c1t0d0                   USB      SanDisk 3.2Gen1   28.67 GiB   yes no
...
need not be the `zones` pool but can be. To enable:
